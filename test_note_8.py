...
print('=' * 80)
print('VERIFICATION: EC avec note < 8 (strictement inférieur)')
print('=' * 80)

for row in donnees['rows']:
    note = row['note']
    if note is not None and note < 8:
        ...
        print(f"Crédit compté: {'OUI' if (note >= 10 or (note >= 8 and row.get('compensated', False))) else 'NON'}")
        print('-' * 80)

print('\n' + '=' * 80)
print('VERIFICATION: EC avec note >= 8 et < 10')
print('=' * 80)
...
print(f"\nMoyenne générale: {donnees['moyenne']}")
print(f"Moyenne Cat A: {donnees['moyenne_cat_a']}")
print(f"Moyenne Cat B: {donnees['moyenne_cat_b']}")
